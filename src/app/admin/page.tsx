...
import { Plus, Upload, Archive, RotateCcw } from "lucide-react";
...
// ── Design palette (matches design file) ──────────────────────────────────
...
const RevTip = ({ active, payload, label }: { active?: boolean; payload?: {value:number}[]; label?: string }) => {
  ...
  return (
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:4, padding:"8px 12px" }}>
      ...
      <div style={{ fontSize:16, fontWeight:600, color:C.dark, marginTop:3 }}>
...
export default function AdminPage() {
  ...
  const editProduct = (id: number, key: keyof Product) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (!authed) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center",
      background:C.loginBg, padding:24, fontFamily:"'Inter Tight',system-ui,sans-serif" }}>
      <div style={{ width:"100%", maxWidth:360, background:C.card, border:`1px solid ${C.border}`,
        ...
        <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#3b4a42", marginBottom:6 }}>Email</label>
        <input value={user} onChange={e=>setUser(e.target.value)} placeholder="you@bottlemoodi.com"
          style={{ width:"100%", boxSizing:"border-box", border:`1px solid ${C.border}`, borderRadius:4,
            padding:"10px 12px", fontSize:14, outline:"none", marginBottom:14, background:"#fff" }}
          onKeyDown={e=>e.key==="Enter"&&setAuthed(true)}/>

        <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#3b4a42", marginBottom:6 }}>Password</label>
        <input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••"
          style={{ width:"100%", boxSizing:"border-box", border:`1px solid ${C.border}`, borderRadius:4,
            padding:"10px 12px", fontSize:14, outline:"none", marginBottom:18, background:"#fff" }}
          onKeyDown={e=>e.key==="Enter"&&setAuthed(true)}/>

        <button onClick={()=>setAuthed(true)}
          style={{ width:"100%", cursor:"pointer", border:"none", background:C.dark, color:C.card,
            fontSize:14, fontWeight:600, padding:11, borderRadius:4 }}>
          Sign in
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight:"100vh", display:"grid", gridTemplateColumns:"216px 1fr",
      fontFamily:"'Inter Tight',system-ui,sans-serif", color:C.dark }}>

      {/* Sidebar */}
      <aside style={{ background:C.sidebar, color:C.sideText, padding:"20px 14px",
        display:"flex", flexDirection:"column", gap:22 }}>
        <div style={{ padding:"0 8px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ fontFamily:"var(--font-bakbak)", fontSize:16, fontWeight:400,
              color:C.card, letterSpacing:".02em" }}>BOTTLEMOODI</div>
            <CapLogo size={30}/>
          </div>
        ...
        <div style={{ marginTop:"auto", padding:"0 8px" }}>
          ...
          <button onClick={()=>{ setAuthed(false); setPass(""); }}
      ...
      {/* Main */}
      <main style={{ background:C.bg, padding:"26px 30px 60px", maxWidth:1180 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:16,
          borderBottom:`1px solid ${C.border}`, paddingBottom:16 }}>
          <h1 style={{ margin:0, fontFamily:"var(--font-bakbak)", fontSize:24, fontWeight:400,
            letterSpacing:".01em" }}>
            {TABS.find(t=>t[0]===tab)?.[1]}
          </h1>
          <div style={{ fontSize:12.5, color:C.muted }}>{saved}</div>
        </div>
        ...
        {tab==="dash" && <>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginTop:22 }}>
            {[
              ...
              <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`,
                ...
                <div style={{ fontSize:11.5, color:C.muted, fontWeight:600, letterSpacing:".04em" }}>{label}</div>
                <div style={{ fontSize:26, fontWeight:600, marginTop:8, letterSpacing:"-.02em" }}>{value}</div>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:6,
            padding:"18px 20px 10px", marginTop:16 }}>
            <div style={{ fontSize:13, fontWeight:600, marginBottom:14 }}>Revenue — last 30 days</div>
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={REVENUE_30} margin={{ top:4, right:4, bottom:0, left:0 }}>
                ...
                <XAxis dataKey="d" tick={{ fontSize:10, fill:C.faint }} tickLine={false} axisLine={false} interval={3}/>
                ...
                <Area type="monotone" dataKey="rev" stroke={C.red} strokeWidth={2}
                  fill="url(#revG)" dot={false} activeDot={{ r:4, fill:C.red, stroke:C.card, strokeWidth:2 }}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>

          <div style={{ display:"grid", gridTemplateColumns:"1.3fr 1fr", gap:16, marginTop:16 }}>
            {/* Weekly units bar chart */}
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:6, padding:18 }}>
              <div style={{ fontSize:13, fontWeight:600, marginBottom:14 }}>Units sold — last 7 days</div>
            ...
            {/* Top prints + order status donut */}
            <div style={{ display:"grid", gap:16 }}>
              <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:6, padding:18 }}>
                <div style={{ fontSize:13, fontWeight:600, marginBottom:12 }}>Top prints</div>
                <div style={{ display:"grid", gap:10 }}>
                  {TOP_PRINTS.map(t => (
                    <div key={t.title} style={{ display:"flex", justifyContent:"space-between",
                      ...
                      <span>{t.title}</span>
                      <span style={{ color:C.muted }}>{t.units}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                  <div style={{ position:"relative", flexShrink:0 }}>
                    <PieChart width={100} height={100}>
                      <Pie data={STATUS_PIE} cx={50} cy={50} innerRadius={32} outerRadius={48}
                    ...
                    <div style={{ position:"absolute", top:"50%", left:"50%",
                      ...
                      <div style={{ fontSize:15, fontWeight:700 }}>312</div>
                      <div style={{ fontSize:8.5, color:C.faint, fontWeight:600 }}>ORDERS</div>
                    </div>
                  </div>
        ...
        {tab==="products" && <>
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:6, overflow:"hidden" }}>
            <div style={{ display:"grid", gridTemplateColumns:"64px 2fr 1.4fr 1fr .8fr .9fr 1.1fr",
              gap:12, padding:"11px 16px", background:C.thead, borderBottom:`1px solid ${C.border}`,
              fontSize:11, fontWeight:700, letterSpacing:".06em", color:C.muted }}>
              <span>ART</span><span>TITLE</span><span>TAMIL LINE</span>
              <span>PRICE ₹</span><span>STOCK</span><span>STATUS</span><span>ACTIONS</span>
            </div>

            {products.map(p => (
              <div key={p.id}
                style={{ display:"grid", gridTemplateColumns:"64px 2fr 1.4fr 1fr .8fr .9fr 1.1fr",
                  gap:12, padding:"11px 16px", borderBottom:`1px solid ${C.rowBorder}`,
                ...
                <div>
                  ...
                  <button onClick={()=>fileRefs.current[p.id]?.click()}
                    style={{ cursor:"pointer", width:44, height:58, border:`1px dashed ${C.border}`,
                      borderRadius:3, background:"repeating-linear-gradient(38deg,#e9eddf 0 5px,#f5f7ee 5px 10px)",
                      fontSize:9, color:C.faint, padding:0, overflow:"hidden",
                      display:"flex", alignItems:"center", justifyContent:"center" }}
                    title="Click to upload image">
                    {p.imageUrl
                      ? <img src={p.imageUrl} alt={p.title} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
                      : <Upload size={12} color={C.faint}/>}
                  </button>
                </div>

                {(["title","tamil","price","stock"] as const).map(key => (
                  <input key={key} value={String(p[key])}
                    onChange={editProduct(p.id, key)}
                    style={{ border:"1px solid transparent", borderRadius:3, padding:"7px 8px",
                      fontSize:13.5, background:"transparent", outline:"none", width:"100%",
                    ...
                    onFocus={e=>{e.target.style.borderColor=C.red; e.target.style.background=C.card;}}
                ...
                <button onClick={()=>{ setProducts(ps=>ps.map(x=>x.id===p.id?{...x,live:!x.live}:x)); flash(p.live?"Archived":"Restored"); }}
                  ...
                  {p.live ? <><Archive size={11}/> Archive</> : <><RotateCcw size={11}/> Restore</>}
                </button>
              </div>
            ))}
          </div>
          <div style={{ fontSize:12.5, color:C.faint, marginTop:10 }}>
            Image slots upload to Cloudinary when <code style={{ background:C.thead, padding:"1px 4px", borderRadius:2 }}>CLOUDINARY_*</code> env vars are set.
          </div>
        </>}
        ...
        {tab==="orders" && (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:6,
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1.6fr 1.9fr .9fr 1.5fr",
              gap:12, padding:"11px 16px", background:C.thead, borderBottom:`1px solid ${C.border}`,
              fontSize:11, fontWeight:700, letterSpacing:".06em", color:C.muted }}>
            ...
            {orders.map((o,i) => (
              <div key={o.id}
                style={{ display:"grid", gridTemplateColumns:"1fr 1.6fr 1.9fr .9fr 1.5fr",
                  gap:12, padding:"12px 16px", borderBottom:`1px solid ${C.rowBorder}`,
                  alignItems:"center", fontSize:13.5 }}>
                <span style={{ fontFamily:"ui-monospace,Menlo,monospace", fontSize:12.5 }}>{o.id}</span>
                <span>{o.name}</span>
        ...
        {/* ══ DISCOUNTS ══ */}
        {tab==="discounts" && <>
          <div style={{ display:"flex", gap:10, margin:"22px 0 14px", flexWrap:"wrap" }}>
            <input value={newCode} onChange={e=>setNewCode(e.target.value.toUpperCase())}
              placeholder="CODE"
              style={{ border:`1px solid ${C.border}`, borderRadius:4, padding:"9px 12px",
                fontSize:13, outline:"none", textTransform:"uppercase", width:160, background:C.card }}/>
            <input value={newPct} onChange={e=>setNewPct(e.target.value)}
              placeholder="% off"
              style={{ border:`1px solid ${C.border}`, borderRadius:4, padding:"9px 12px",
                fontSize:13, outline:"none", width:90, background:C.card }}/>
            <button
